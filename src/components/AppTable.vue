...
import { UserListTableData } from "@/elements/TableArrayData";
...
const {
  ...
  addActive,
  sortingArr,
} = handlePagination(UserListTableData, qtyPage.value);
  ...
  <div class="table-responsive">
    <table class="table style-1" id="ListDatatableView">
      <thead>
        <tr>
          <th
            @click="sortingArr(ind)"
            v-for="({ name }, ind) in tableBtn"
            :key="ind"
          >
            {{ name }}
          </th>
      ...
      <tbody>
        <tr
          v-for="(
            { img, name, email, companyName, status, badge, avatar, className },
            ind
          ) in paginatedData"
          :key="ind"
        >
          ...
          <td>
            <div class="media style-1">
              <img :src="img" class="img-fluid me-2" alt="" v-if="!className" />
              <span :class="className" v-else>{{ avatar }}</span>
    ...
    <div
      class="d-flex align-items-center justify-content-between basic-data_table_pagination"
    >
      <div class="dataTables_info">
        Showing {{ paginationLength - 9 }} to {{ paginationLength }} of
        {{ UserListTableData.length }} entries
      </div>
      <div
      ...
      >
        ...
        ><span>
          <a
            :class="`paginate_button border-0 text-center m-1 ${
              addActive === item ? 'current' : ''
            }`"
